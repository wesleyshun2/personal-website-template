...
export function HeroParallax({
    ...
    return (
        <div className="relative h-[90vh] min-h-[700px] w-full flex items-center justify-center overflow-hidden bg-zinc-50 dark:bg-zinc-950">
            ...
            {/* Full-width Glass Overlay Layer */}
            <div className={`absolute inset-0 z-[5] pointer-events-none ${siteConfig.hero.glassBlur} ${siteConfig.hero.glassOpacityLight} ${siteConfig.hero.glassOpacityDark}`} />

            {/* Content Layer */}
            <motion.div
                style={{ y: textY, opacity }}
                className="relative z-10 flex flex-col items-center justify-center px-8 text-center max-w-6xl mx-auto"
            >
                <h1 className="text-4xl md:text-5xl font-light tracking-tight text-zinc-900 dark:text-zinc-50 mb-10 leading-[1.1] drop-shadow-md">
                    {title}
                </h1>
                <p className="text-lg md:text-2xl text-zinc-700 dark:text-zinc-300 font-light max-w-3xl mx-auto mb-12 drop-shadow-sm">
                    {subtitle}
                </p>
